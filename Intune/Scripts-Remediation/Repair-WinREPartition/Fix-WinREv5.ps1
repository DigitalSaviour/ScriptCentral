<#
.SYNOPSIS
    Shrinks OS drive so WinRE partition will work

.DESCRIPTION
    When the WinRE partition is causing encryption issues this script will remote the recovery partition, shrink the OS
    download the files from Centrality storage blob and activate WinRE partition.

    After a reboot the drive should encrypt and report in correctly to Intune.
...
.AUTHOR(S)
    ...
    Lewis Thompson (lthompson@centrality.com)
...
'@
    ...
    $url1 = "https://centralitybitlocker.blob.core.windows.net/bitlocker/boot.sdi" ##yourlocation##/boot.sdi"
    ...
    # Specify the drive letter associated with your recovery partition
